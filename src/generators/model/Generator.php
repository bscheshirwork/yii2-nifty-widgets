...
class Generator extends \yii\gii\generators\model\Generator
{
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->foreignKeys as $refs) {
            ...
            $rules[] = "[['$attributes'], 'exist', 'skipOnError' => true, 'targetClass' => get_class(Yii::createObject($refClassName::class)), 'targetAttribute' => [$targetAttributes]]";
        }
    ...
    private function generateManyManyRelations($table, $fks, $relations)
    {
        ...
        foreach ($fks as $pair) {
            ...
            $relations[$table0Schema->fullName][$relationName] = [
                "return \$this->hasMany(get_class(Yii::createObject($className1::class)), $link)->viaTable('"
            ...
            $relations[$table1Schema->fullName][$relationName] = [
                "return \$this->hasMany(get_class(Yii::createObject($className0::class)), $link)->viaTable('"
                . $this->generateTableName($table->name) . "', $viaLink);",
                $className0,
                true,
            ];
            ...
            foreach ($db->getSchema()->getTableSchemas($schemaName) as $table) {
                ...
                foreach ($table->foreignKeys as $refs) {
                    ...
                    $relations[$table->fullName][$relationName] = [
                        "return \$this->hasOne(get_class(Yii::createObject($refClassName::class)), $link);",
                        $refClassName,
                        false,
                    ];
                    ...
                    $relations[$refTableSchema->fullName][$relationName] = [
                        "return \$this->" . ($hasMany ? 'hasMany' : 'hasOne') . "(get_class(Yii::createObject($className::class)), $link);",
                        $className,
                        $hasMany,
                    ];
                }
